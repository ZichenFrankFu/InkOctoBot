...
export default function RunnerPage(props: RunnerPageProps) {
  ...
  useEffect(() => {
    setMetaLogs((logs) =>
      [`${new Date().toLocaleTimeString()} [config] 配置已更新，可直接一键启动（未保存也可运行）`, ...logs].slice(0, 200),
    );
  }, [safeConfigVersion]);
  ...
  async function start() {
    ...
    try {
      const res = await apiPost<{ task_id: string }>("/api/tasks/spider/adhoc", safeConfig || {});
    ...
    <div style={{ color: "var(--text-primary)" }}>
      ...
      <section style={{ ...boxStyle, marginBottom: 14 }}>
        <div style={{ display: "flex", gap: 10, alignItems: "center", marginBottom: 10 }}>
          <button disabled={starting} onClick={start} style={buttonStyle}>
            {starting ? "启动中..." : "一键启动爬虫"}
          </button>
          <button onClick={refreshTasks} style={buttonStyle}>
            刷新运行状态
          </button>
        ...
        >
          当前运行参数摘要：platform={safeConfig?.platform || "-"} | rank_key={safeConfig?.rank_key || "ALL"} |
          chapter_count={safeConfig?.chapter_count ?? "-"} | newbook_chapter_count={safeConfig?.newbook_chapter_count ?? "-"}
        </div>
      </section>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "0.9fr 1.1fr", gap: 14 }}>
        ...
        <section style={boxStyle}>
          ...
          {activeTaskId ? (
            <LogViewer taskId={activeTaskId} pollMs={700} height={540} />
          ) : (
            <div style={{ color: "var(--text-secondary)" }}>尚未启动任务</div>
          )}
        </section>
      </div>
    </div>
