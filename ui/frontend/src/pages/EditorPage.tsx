...
export default function EditorPage({ projectId }: { projectId: string }) {
  ...
  const [rewriteBtn, setRewriteBtn] = useState<{ top: number } | null>(null);
  ...
  useEffect(() => {
    ...
    apiGet<any>(`/api/data/editor?project_id=${pid}`).then(d => {
      if (d.volumes?.length) {
        setVols(d.volumes);
        setActiveId(d.volumes[0]?.chapters?.[0]?.id || "");
      }
      setLoaded(true);
  ...
  useEffect(() => { if (ch && loaded) { setContent(ch.content); setTitleVal(ch.title); } setEditingTitle(false); setSelection(null); setRewriteBtn(null); }, [activeId, loaded]);
  ...
  // Text selection — use onMouseUp for reliable detection
  const handleMouseUp = () => {
    const el = textRef.current;
    if (!el) return;
    // Use setTimeout to let the browser finalize selection
    setTimeout(() => {
      ...
      if (s !== undefined && e !== undefined && e > s) {
        ...
        if (txt.trim().length > 0) {
          ...
          const lines = content.substring(0, s).split("\n").length;
          setRewriteBtn({ top: Math.min(lines * 32 + 50, 500) });
          return;
        }
      }
      setSelection(null);
    ...
    <div style={{ display: "flex", height: "100vh", overflow: "hidden" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", background: "var(--paper)", position: "relative" }}>
        ...
        <div style={{ flex: 1, overflow: "hidden", position: "relative" }}>
          <textarea
            ref={textRef} value={content}
            onChange={e => { setContent(e.target.value); setSelection(null); setRewriteBtn(null); }}
          ...
          {rewriteBtn && selection && (
            <div style={{ position: "absolute", top: rewriteBtn.top, left: "50%", transform: "translateX(-50%)", zIndex: 50 }}>
              <button
                style={{ padding: "5px 14px", background: "var(--accent)", color: "white", border: "none", borderRadius: 16, fontSize: 11, fontWeight: 600, cursor: "pointer", boxShadow: "var(--shadow-md)", fontFamily: "var(--font-sans)" }}
                onClick={() => { setAiTab("rewrite"); }}
              >
                ✏️ AI重写 ({selection.text.length}字)
              </button>
            </div>
          )}
        </div>
      ...
      <div style={{ width: rightW, flexShrink: 0, background: "var(--paper-card)", borderLeft: "1px solid var(--ink-100)", display: "flex", flexDirection: "column", overflow: "hidden" }}>
        ...
        <div style={{ display: "flex", borderBottom: "1px solid var(--ink-50)" }}>
          {([["outline", "📋 大纲"], ["inspire", "✨ AI灵感"], ["rewrite", "✏️ AI重写"]] as const).map(([k, l]) =>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "12px 16px" }}>
          ...
          {aiTab === "rewrite" && <RewriteTab selection={selection} prompt={rewritePrompt} onPromptChange={setRewritePrompt} />}
        </div>
      </div>
    </div>
  );
...
function OutlineTab({ outline, onChange }: { outline: string; onChange: (v: string) => void }) {
  return <div>
    ...
    <div style={{ marginTop: 12, fontSize: 11, color: "var(--ink-400)" }}>大纲将作为 Scene Planner 的输入，AI 根据大纲拆分场景并生成内容。也会自动同步到「剧情线」页面。</div>
  </div>;
}
...
function InspireTab() {
  ...
  return <div>
    ...
    {PIPELINE_DEMO.map((p, i) => <div key={i} style={{ marginBottom: 10 }}>
      <div onClick={() => setExpanded(expanded === i ? null : i)} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 10px", background: p.status === "done" ? "var(--jade-light)" : p.status === "active" ? "var(--gold-light)" : "var(--ink-50)", borderRadius: 6, cursor: "pointer" }}>
...
function RewriteTab({ selection, prompt, onPromptChange }: { selection: { start: number; end: number; text: string } | null; prompt: string; onPromptChange: (v: string) => void }) {
  return <div>
    <div style={{ fontSize: 12, fontWeight: 600, color: "var(--ink-500)", marginBottom: 8 }}>AI 重写选中文本</div>
    {selection ? <>
      ...
      <div style={{ fontSize: 11, color: "var(--ink-400)", marginBottom: 10 }}>选中了 {selection.text.length} 字（位置 {selection.start}-{selection.end}）</div>

      {/* Prompt input */}
      <div style={{ marginBottom: 12 }}>
        <label style={{ fontSize: 11, fontWeight: 600, color: "var(--ink-500)", display: "block", marginBottom: 4 }}>重写指令（可选）</label>
        <textarea
          value={prompt} onChange={e => onPromptChange(e.target.value)}
          rows={3}
          placeholder={"告诉 AI 你想怎么改…\n例如：更紧张、加入内心描写、换成第一人称"}
          style={{ width: "100%", padding: "8px 10px", border: "1px solid var(--ink-200)", borderRadius: 6, fontSize: 12, fontFamily: "var(--font-sans)", outline: "none", resize: "vertical", lineHeight: 1.5 }}
        />
      </div>
      ...
      <p>在编辑器中<strong>选中文本</strong>，将出现「AI重写」按钮</p>
      <p style={{ fontSize: 11, marginTop: 6, lineHeight: 1.6 }}>支持选中任意长度的文本：单词、句子、段落均可</p>
    </div>}
  </div>;
}