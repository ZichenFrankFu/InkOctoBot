...
export default function WorldBookPage({ projectId, projects }: { projectId: string; projects: Project[] }) {
  ...
  const load = useCallback(() => {
    setLoading(true);
    apiGet<{ items: Entry[] }>(`/api/data/worldbook${projectId ? `?project_id=${projectId}` : ""}`)
  ...
  const create = async () => {
    const e = await post("/api/data/worldbook", { category: cat || "力量体系", title: "新条目", project_id: projectId });
    setItems([...items, e]);
    setEditing(e);
    setDirty(false);
  };
    ...
    <div style={{ display: "flex", height: "100vh", overflow: "hidden" }}>
      ...
      <div style={{ width: 320, flexShrink: 0, background: "var(--paper-card)", borderRight: "1px solid var(--ink-100)", display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ padding: "10px 12px", borderBottom: "1px solid var(--ink-50)", display: "flex", gap: 4, flexWrap: "wrap" }}>
          <button onClick={() => setCat("")} style={{ padding: "4px 10px", borderRadius: 12, border: cat === "" ? "1px solid var(--accent)" : "1px solid var(--ink-200)", background: cat === "" ? "var(--accent-muted)" : "transparent", color: cat === "" ? "var(--accent)" : "var(--ink-500)", fontSize: 11, cursor: "pointer" }}>
            全部 ({items.length})
          </button>
          {CATS.map(c => (
            <button key={c.n} onClick={() => setCat(c.n)} style={{ padding: "4px 10px", borderRadius: 12, border: cat === c.n ? "1px solid var(--accent)" : "1px solid var(--ink-200)", background: cat === c.n ? "var(--accent-muted)" : "transparent", color: cat === c.n ? "var(--accent)" : "var(--ink-500)", fontSize: 11, cursor: "pointer" }}>
              {c.i} {c.n} ({cc[c.n] || 0})
            </button>
          ))}
        </div>
        <div style={{ flex: 1, overflowY: "auto" }}>
          {loading ? <div className="loading"><div className="loading-spinner" /></div> : filtered.length === 0 ?
            ...
            filtered.map(e => (
              <div key={e.id} onClick={() => { setEditing(e); setDirty(false); }} style={{ padding: "12px 16px", borderBottom: "1px solid var(--ink-50)", cursor: "pointer", background: editing?.id === e.id ? "var(--accent-muted)" : "transparent" }}>
            ...
            <div className="card" style={{ marginBottom: 20 }}>
              ...
              <div className="card-body">
                <div style={{ marginBottom: 12 }}>
                  <label style={{ fontSize: 12, fontWeight: 600, color: "var(--ink-500)", display: "block", marginBottom: 4 }}>标题</label>
                  <input value={editing.title} onChange={e => u("title", e.target.value)} style={{ width: "100%", padding: "8px 12px", border: "1px solid var(--ink-200)", borderRadius: 6, fontSize: 14, outline: "none", fontWeight: 600 }} />
                </div>
                <div style={{ marginBottom: 12 }}>
                  <label style={{ fontSize: 12, fontWeight: 600, color: "var(--ink-500)", display: "block", marginBottom: 4 }}>分类</label>
                  <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                    {CATS.map(c => (
                      <button key={c.n} onClick={() => u("category", c.n)} style={{ padding: "6px 14px", borderRadius: 20, border: editing.category === c.n ? "2px solid var(--accent)" : "1px solid var(--ink-200)", background: editing.category === c.n ? "var(--accent-muted)" : "transparent", color: editing.category === c.n ? "var(--accent)" : "var(--ink-600)", fontSize: 12, cursor: "pointer" }}>
                        {c.i} {c.n}
                      </button>
                    ))}
                  </div>
                ...
                <div style={{ marginBottom: 12 }}>
                  <label style={{ fontSize: 12, fontWeight: 600, color: "var(--ink-500)", display: "block", marginBottom: 4 }}>标签</label>
                  <input value={(editing.tags || []).join(", ")} onChange={e => u("tags", e.target.value.split(",").map(s => s.trim()).filter(Boolean))} style={{ width: "100%", padding: "8px 12px", border: "1px solid var(--ink-200)", borderRadius: 6, fontSize: 13, outline: "none" }} />
                </div>
              </div>
            </div>
            <div className="card">
