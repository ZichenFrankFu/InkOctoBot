...
Usage:
    python -m scripts.migrate_to_storyland_state <project_id> \
...
# Allow running as `python -m scripts.migrate_to_storyland_state`
HERE = Path(__file__).resolve()
PROJECT_ROOT = HERE.parent.parent
if str(PROJECT_ROOT) not in sys.path:
    sys.path.insert(0, str(PROJECT_ROOT))

from knowledge.storyland_state.migrate import migrate_all  # noqa: E402
from knowledge.storyland_state.store import StorylandStateStore  # noqa: E402
...
def main() -> int:
    ...
    store = StorylandStateStore(args.project_id, db_path=args.db_path)
