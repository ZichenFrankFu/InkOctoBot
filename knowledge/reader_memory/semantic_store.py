...
Deprecated values (kept readable for old rows, no new writes):
  ...
  - ``character_state`` → use ``emotion_arcs`` (relations live on
                          ``character_snapshots.relations_overrides``)
...
logger = logging.getLogger("inkoctobot.knowledge.reader_memory.semantic_store")
...
class SemanticMemory:
    ...
    def store_permanent_fact(self, project_id: str, content: str,
        ...
        Callers should emit StatePatch via StorylandStateStore.apply_deltas.
        """
        logger.debug(
    ...
    def store_character_state(self, project_id: str, character_name: str,
        ...
        """Deprecated (v2.1): use emotion_arcs / character_snapshots instead."""
        logger.debug(
