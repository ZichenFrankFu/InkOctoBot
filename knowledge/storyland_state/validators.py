...
callable from StorylandStateStore.apply_deltas (via validate=True) or
...
from knowledge.storyland_state.schemas import (
    HookDelta, HookStatus, NumericalReconciliation, StorylandStateDeltas,
...
logger = logging.getLogger("inkoctobot.knowledge.storyland_state.validators")
...
def _resolve_known_characters(store: "StorylandStateStore") -> set[str] | None:
...
def _rule_hook_no_duplicate_id(deltas: StorylandStateDeltas) -> list[ValidationIssue]:
    seen: set[str] = set()
...
def _rule_ledger_closed_equation(deltas: StorylandStateDeltas) -> list[ValidationIssue]:
...
def _rule_state_no_conflicting_triple(deltas: StorylandStateDeltas) -> list[ValidationIssue]:
...
def _rule_xref_character_in_emotion(
    deltas: StorylandStateDeltas, known: set[str],
) -> list[ValidationIssue]:
    out: list[ValidationIssue] = []
...
def _rule_chapter_monotonic(
    conn: sqlite3.Connection, project_id: str, deltas: StorylandStateDeltas,
) -> list[ValidationIssue]:
    row = conn.execute(
