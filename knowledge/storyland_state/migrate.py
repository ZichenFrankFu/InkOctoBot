...
Sources:
  ...
  - character_cards       -> truth_current_state (location/mood only;
                              relations now live on character_snapshots)
  - chapter_summaries     -> emotion_arcs (heuristic diff between adjacent
                              chapters' character_states_json)

Each migration emits one or more StorylandStateDeltas batches and applies them
...
from knowledge.storyland_state.schemas import (
    ...
    HookImportance, StatePatch, StorylandStateDeltas,
)
from knowledge.storyland_state.store import StorylandStateStore

logger = logging.getLogger("inkoctobot.knowledge.storyland_state.migrate")
...
def migrate_foreshadowing_json(
    store: StorylandStateStore, *, dry_run: bool = False,
) -> MigrationReport:
    ...
    # Group hooks by origin_chapter so each StorylandStateDeltas applies to one chapter.
    ...
    for chapter_num, hooks in sorted(by_chapter.items()):
        deltas = StorylandStateDeltas(chapter_num=chapter_num, hook_deltas=hooks)
        if dry_run:
...
# ──────────────────────────────────────────────────────────────
# 3. character_cards → truth_current_state
# ──────────────────────────────────────────────────────────────

def migrate_character_cards(
    store: StorylandStateStore, *, dry_run: bool = False,
) -> MigrationReport:
    """Migrate character card location/mood into truth_current_state.

    Relations on character cards are no longer migrated — pairwise
    relationships now live on ``character_snapshots.relations_overrides``
    (LOADER_SPEC Loader 5, Batch 5). Pre-existing relation data is left
    in the card JSON for the snapshot-import step to consume later.
    """
...
def migrate_chapter_summaries(
    ...
    for chapter_num, arcs in sorted(arc_entries_by_chap.items()):
        deltas = StorylandStateDeltas(chapter_num=chapter_num, emotion_arc_entries=arcs)
        if not dry_run:
            res = store.apply_deltas(deltas, allow_backfill=True)
            report.apply_results.append(res)
        report.batches_applied += 1
