"""Tests for knowledge.storyland_state.validators (C3): 12 cross-file rules + state audit."""
...
from knowledge.storyland_state.schemas import (
    ...
    SubplotUpdate, StorylandStateDeltas,
)
from knowledge.storyland_state.store import StorylandStateStore
from knowledge.storyland_state.validators import validate_deltas, validate_state
...
class TestWithinDeltaRules:
    ...
    def test_hook_no_duplicate_id_pos(self, store: StorylandStateStore):
    ...
    def test_hook_no_duplicate_id_neg(self, store: StorylandStateStore):
        deltas = StorylandStateDeltas(
            chapter_num=1,
            hook_deltas=[
                HookDelta(hook_id="h1", description="a", action="new"),
    ...
    def test_ledger_closed_equation_violation(self, store: StorylandStateStore):
        deltas = StorylandStateDeltas(
            chapter_num=1,
    ...
    def test_ledger_closed_equation_ok(self, store: StorylandStateStore):
        deltas = StorylandStateDeltas(
            chapter_num=1,
            particle_reconciliations=[
                NumericalReconciliation(
    ...
    def test_ledger_set_operation_skips_equation(self, store: StorylandStateStore):
    ...
    def test_state_conflicting_triple_warns(self, store: StorylandStateStore):
        deltas = StorylandStateDeltas(
            chapter_num=3,
...
class TestCharacterXref:
    ...
    def test_emotion_unknown_character_errors(self, store: StorylandStateStore):
        deltas = StorylandStateDeltas(
            chapter_num=1,
            emotion_arc_entries=[
    ...
    def test_emotion_known_character_ok(self, store: StorylandStateStore):
        deltas = StorylandStateDeltas(
            chapter_num=1,
            emotion_arc_entries=[
    ...
    def test_xref_skipped_when_known_characters_none(self, store: StorylandStateStore):
...
class TestDbRules:
    ...
    def test_chapter_monotonic_violation(self, store: StorylandStateStore, db_path: str):
        store.apply_deltas(StorylandStateDeltas(chapter_num=5))
        issues = validate_deltas(
            store, StorylandStateDeltas(chapter_num=3),
            known_characters=KNOWN_CHARS,
        )
        rule_ids = _issue_ids(issues)
        assert "chapter.monotonic" in rule_ids

    def test_chapter_monotonic_ok(self, store: StorylandStateStore):
        store.apply_deltas(StorylandStateDeltas(chapter_num=5))
        issues = validate_deltas(
            store, StorylandStateDeltas(chapter_num=6),
            known_characters=KNOWN_CHARS,
        )
        assert "chapter.monotonic" not in _issue_ids(issues)

    def test_chapter_monotonic_allow_backfill_skips(self, store: StorylandStateStore):
        store.apply_deltas(StorylandStateDeltas(chapter_num=5))
        issues = validate_deltas(
            store, StorylandStateDeltas(chapter_num=3),
    ...
    def test_hook_orphan_progress(self, store: StorylandStateStore):
        deltas = StorylandStateDeltas(
            chapter_num=1,
            hook_deltas=[
    ...
    def test_hook_orphan_progress_satisfied_by_new_in_batch(
        self, store: StorylandStateStore,
    ):
        deltas = StorylandStateDeltas(
            chapter_num=1,
            hook_deltas=[
    ...
    def test_hook_transition_against_terminal_errors(self, store: StorylandStateStore):
    ...
    def test_ledger_matches_current_violation(self, store: StorylandStateStore):
    ...
    def test_ledger_matches_current_no_prior_record(self, store: StorylandStateStore):
    ...
    def test_xref_hook_in_subplot_violation(self, store: StorylandStateStore):
        deltas = StorylandStateDeltas(
            chapter_num=1,
            subplot_updates=[
    ...
    def test_xref_hook_in_subplot_satisfied_by_new(self, store: StorylandStateStore):
        deltas = StorylandStateDeltas(
            chapter_num=1,
    ...
    def test_subplot_hook_resolved_should_advance(self, store: StorylandStateStore):
...
class TestValidateState:
    ...
    def test_validate_state_clean(self, store: StorylandStateStore):
        store.apply_deltas(StorylandStateDeltas(
            chapter_num=1,
