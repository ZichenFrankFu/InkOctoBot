...
from knowledge.storyland_state.markdown_renderer import (
...
from knowledge.storyland_state.schemas import (
    ...
    NumericalReconciliation, StatePatch, SubplotStatus,
    SubplotUpdate, StorylandStateDeltas, StorylandStateKind,
)
...
@pytest.fixture
def seeded_store(db_path: str) -> StorylandStateStore:
    store = StorylandStateStore("p1", db_path=db_path)
...
class TestCurrentStateQuery:
    ...
    def test_query_all(self, seeded_store: StorylandStateStore):
        rows = seeded_store.query_current_state()
        assert len(rows) == 2

    def test_query_by_subject(self, seeded_store: StorylandStateStore):
        rows = seeded_store.query_current_state(subject="张远")
        assert all(r["subject"] == "张远" for r in rows)

    def test_query_chapter_window(self, seeded_store: StorylandStateStore):
    ...
    def test_query_after_supersede(self, db_path: str, seeded_store: StorylandStateStore):
        # add a newer triple for (张远, 在) at ch 8 — old should be closed
        seeded_store.apply_deltas(StorylandStateDeltas(
            chapter_num=8,
            current_state_patches=[
...
class TestLedgerQuery:
    ...
    def test_current_value(self, seeded_store: StorylandStateStore):
        assert seeded_store.query_ledger("张远", "灵气") == 100

    def test_current_value_unknown_returns_none(self, seeded_store: StorylandStateStore):
        assert seeded_store.query_ledger("unknown", "灵气") is None

    def test_as_of_chapter_filters_history(self, seeded_store: StorylandStateStore):
    ...
    def test_list_ledger_entries(self, seeded_store: StorylandStateStore):
...
class TestHookQuery:
    ...
    def test_query_by_status(self, seeded_store: StorylandStateStore):
    ...
    def test_list_pressured_not_overdue(self, seeded_store: StorylandStateStore):
...
def test_query_chapter_summary(seeded_store: StorylandStateStore):
    summ = seeded_store.query_chapter_summary(chapter_num=5)
    assert summ is not None
    assert summ["summary_text"] == "主角下山"


def test_query_chapter_summary_missing(seeded_store: StorylandStateStore):
    assert seeded_store.query_chapter_summary(chapter_num=999) is None


def test_list_chapter_summaries(seeded_store: StorylandStateStore):
...
def test_query_subplot_threads(seeded_store: StorylandStateStore):
...
def test_query_emotion_arc(seeded_store: StorylandStateStore):
...
class TestStoreRender:
    ...
    def test_render_for_prompt_returns_markdown(
        self, seeded_store: StorylandStateStore,
    ):
        out = seeded_store.render_for_prompt(
            StorylandStateKind.pending_hooks, chapter_num=5,
        )
        assert "Pending Hooks" in out
        assert "h_玉佩" in out

    def test_render_bundle_default_kinds(self, seeded_store: StorylandStateStore):
        bundle = seeded_store.render_bundle_for_prompt(
            chapter_num=5, characters=["张远", "李清漪"],
        )
        # all 6 kinds returned (character_matrix removed in v3.0)
        assert len(bundle) == 6
        for k, md in bundle.items():
            assert "---\n" in md, f"frontmatter missing for {k}"

    def test_render_bundle_with_kinds_filter(self, seeded_store: StorylandStateStore):
        bundle = seeded_store.render_bundle_for_prompt(
            chapter_num=5,
            kinds=[StorylandStateKind.pending_hooks, StorylandStateKind.particle_ledger],
        )
        assert set(bundle.keys()) == {
            StorylandStateKind.pending_hooks, StorylandStateKind.particle_ledger,
        }

    def test_export_markdown_writes_files(
        self, seeded_store: StorylandStateStore, tmp_path: Path,
    ):
    ...
    def test_render_filters_by_characters(self, seeded_store: StorylandStateStore):
        ...
        # filter to 张远 only
        out = seeded_store.render_for_prompt(
            StorylandStateKind.emotional_arcs, chapter_num=6,
            characters=["张远"],
        )
        assert "张远" in out
