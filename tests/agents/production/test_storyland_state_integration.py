"""
Tests for agents/production/storyland_state_integration.py.

Covers the 3 Writer × StorylandState integration points from
docs/storyland_state_system.md §9:
  - build_state_context        (Phase 1 prompt assembly)
  - list_pressured_hooks_text  (pressured-hook injection)
  - extract_and_apply_state_deltas (Phase 2 settlement + audit gate)
"""
...
class TestBuildTruthContext(unittest.TestCase):
    ...
    def test_empty_project_returns_frontmatter_only(self) -> None:
        ...
        db = _fresh_db()
        out = ti.build_state_context("p1", db, chapter_num=1, characters=[])
    ...
    def test_renders_when_data_exists(self) -> None:
        ...
        with sqlite3.connect(db) as c:
            ...
            c.commit()
        out = ti.build_state_context("p1", db, chapter_num=5)
...
class TestExtractStorylandStateDeltas(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_fenced_with_json_tag_parses(self) -> None:
        ...
        deltas = await ti.extract_state_deltas(router, "x", chapter_num=2)
        self.assertIsNotNone(deltas)
        self.assertEqual(len(deltas.current_state_patches), 1)

    # NOTE: score-clamping test removed with character_matrix in v3.0.
    # Relation parsing has moved to character_snapshots (Batch 5);
    # ledger.delta clamping is exercised by ledger-specific tests.


class TestExtractAndApplyStorylandStateDeltas(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_idempotent_apply(self) -> None:
        ...
        r1 = await ti.extract_and_apply_state_deltas(
            router, "x", project_id="p1", db_path=db, chapter_num=2,
        )
        r2 = await ti.extract_and_apply_state_deltas(
            router, "x", project_id="p1", db_path=db, chapter_num=2,
        )
