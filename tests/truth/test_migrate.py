...
class TestForeshadowingMigration:
    ...
    def test_no_file_marks_skipped(
        self, store: TruthFileStore, monkeypatch, tmp_path: Path,
    ):
    ...
    def test_two_items_split_by_origin(
        self, store: TruthFileStore, db_path: str, monkeypatch, tmp_path: Path,
    ):
        path = tmp_path / "fs.json"
...
class TestCharacterCardsMigration:
    ...
    def test_no_characters_skipped(self, store: TruthFileStore, monkeypatch):
    ...
    def test_state_patches_emitted_for_location_mood(
        self, store: TruthFileStore, db_path: str, monkeypatch,
    ):
        monkeypatch.setattr(M, "_character_rows", lambda _: [
    ...
    def test_relation_list_form(
        self, store: TruthFileStore, db_path: str, monkeypatch,
    ):
        monkeypatch.setattr(M, "_character_rows", lambda _: [
            {"name": "张远",
             "relationships": [
                 {"target_name": "李清漪", "label": "盟友"},
                 {"target_name": "黑衣人", "label": "敌对"},
             ]},
        ])
        # Provide enum→score mapping
        monkeypatch.setattr(M, "_config_relation_map", lambda: {
            "盟友": {"sentiment": 70, "trust": 70},
            "敌对": {"sentiment": -60, "trust": 5},
        })
        rep = M.migrate_character_cards(store)
        assert rep.success
        assert rep.rows_emitted["relation_updates"] == 2
        with sqlite3.connect(db_path) as conn:
            rows = {
                (r[0], r[1]): (r[2], r[3], r[4])
                for r in conn.execute(
                    "SELECT character_a, character_b, relation_type, "
                    "sentiment_score, trust_level FROM character_relations "
                    "WHERE project_id='p1'",
                )
            }
        assert ("张远", "李清漪") in rows
        assert rows[("张远", "李清漪")] == ("盟友", 70, 70)
        assert rows[("张远", "黑衣人")] == ("敌对", -60, 5)

    def test_relation_dict_form(
        self, store: TruthFileStore, db_path: str, monkeypatch,
    ):
        monkeypatch.setattr(M, "_character_rows", lambda _: [
            {"name": "A",
             "relationships": {"B": "盟友", "C": "敌对"}},
        ])
        monkeypatch.setattr(M, "_config_relation_map", lambda: {
            "盟友": {"sentiment": 70, "trust": 70},
            "敌对": {"sentiment": -60, "trust": 5},
...
class TestChapterSummariesMigration:
    ...
    def test_too_few_summaries_skipped(self, store: TruthFileStore):
...
def test_migrate_all_runs_all_sources(store: TruthFileStore, monkeypatch):
...
def test_migrate_all_subset(store: TruthFileStore, monkeypatch):
    monkeypatch.setattr(M, "migrate_foreshadowing_json",
