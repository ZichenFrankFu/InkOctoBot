...
def test_truth_file_kind_has_seven_members():
    assert len(list(TruthFileKind)) == 7
    expected = {
        "current_state", "particle_ledger", "pending_hooks",
        "chapter_summaries", "subplot_board", "emotional_arcs",
        "character_matrix",
    }
    assert {k.value for k in TruthFileKind} == expected
...
def test_relation_sentiment_score_range():
    RelationUpdate(
        character_a="A", character_b="B", relation_type="盟友",
        sentiment_score=100, trust_level=50,
    )
    with pytest.raises(ValidationError):
        RelationUpdate(
            character_a="A", character_b="B", relation_type="盟友",
            sentiment_score=101, trust_level=50,
        )
    with pytest.raises(ValidationError):
        RelationUpdate(
            character_a="A", character_b="B", relation_type="盟友",
...
def test_relation_trust_level_range():
    RelationUpdate(
        character_a="A", character_b="B", relation_type="盟友",
    ...
    RelationUpdate(
        character_a="A", character_b="B", relation_type="盟友",
        sentiment_score=0, trust_level=100,
    )
    with pytest.raises(ValidationError):
        RelationUpdate(
            character_a="A", character_b="B", relation_type="盟友",
            sentiment_score=0, trust_level=101,
        )
    with pytest.raises(ValidationError):
        RelationUpdate(
            character_a="A", character_b="B", relation_type="盟友",
            sentiment_score=0, trust_level=-1,
        )
...
def test_truth_deltas_defaults_empty():
    ...
    assert d.character_relation_updates == []
...
def test_truth_deltas_full_bundle_constructs():
    d = TruthDeltas(
        ...
        character_relation_updates=[
            RelationUpdate(character_a="张远", character_b="宗主",
                           relation_type="师徒", sentiment_score=-10,
                           trust_level=20),
        ],
    )
