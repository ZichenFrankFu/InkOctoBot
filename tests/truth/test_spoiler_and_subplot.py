...
A4: SubplotUpdate flows through extract_truth_deltas + applies to
...
from knowledge.truth.schemas import (
    HookDelta, HookImportance, SubplotStatus, SubplotUpdate, TruthDeltas,
)
...
class TestSpoilerFlagPersistence(unittest.TestCase):
    ...
    def test_hook_delta_with_spoiler_persists_flag(self) -> None:
        db = _fresh_db()
        store = TruthFileStore("p1", db_path=db)
...
class TestBuildTruthContextSpoilerPlumbing(unittest.TestCase):
    """Verify spoiler filter plumbs through to truth_integration."""
    ...
    def test_pov_character_passes_through(self) -> None:
        ...
        all_view = ti.build_truth_context("p1", db, chapter_num=2, pov_character=None)
        self.assertIn("A 知道的事", all_view)
        self.assertIn("A 不知道的事", all_view)

        # POV=A only sees the public one
        a_view = ti.build_truth_context(
