"""
Tests for ReaderMemoryConsolidator → Truth Files emission (v2).
...
Verifies the consolidator:
  ...
  - applies via StorylandStateStore.apply_deltas
...
class TestConsolidatorEmitsStorylandStateDeltas(unittest.IsolatedAsyncioTestCase):
    ...
    async def asyncSetUp(self) -> None:
        ...
        self.consolidator = ReaderMemoryConsolidator(
            router=self.router,
...
class TestReaderMemoryManagerForeshadow(unittest.TestCase):
    ...
    def test_get_unresolved_foreshadowing_reads_pending_hooks(self) -> None:
        ...
        mgr = ReaderMemoryManager(db_path=self.db)
        mgr.set_project("p1")
