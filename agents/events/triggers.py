...
README §2.6.2: Each agent registers triggers that fire when
specific conditions are met, with cooldown to prevent spam.
"""
...
from agents.events.event_types import Event, AgentSuggestion
from agents.events.event_bus import EventBus

logger = logging.getLogger("inkoctobot.agents.events.triggers")
...
class TriggerRegistry:
    ...
    def register(self, trigger: TriggerRule) -> None:
        ...
        logger.info("Registered trigger: %s on %s", trigger.agent_name, trigger.event_types)
