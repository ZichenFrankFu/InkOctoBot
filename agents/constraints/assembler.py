...
README §2.5: Five priority levels, assembled so highest-priority constraints
are positioned at the front of the attention window.

Priority order: 世界观硬规则 > 知识隔离 > 情节约束 > 叙事风格 > 修辞风格
"""
...
class ConstraintAssembler:
    ...
    def get_constraints(self, project_id: str, rule_type: str | None = None) -> list[dict]:
    ...
    def assemble(
        ...
        *,
        ...
        if include_types:
            constraints = [c for c in constraints if c["rule_type"] in include_types]
        ...
        # Add scene-specific constraints (treated as plot_constraint)
        if scene_constraints:
            by_priority.setdefault(3, []).extend([
                {"description": sc.get("description", ""),
                 ...
                 "rule_type": "plot_constraint", "priority": 3}
